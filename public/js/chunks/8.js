  ...
  methods: {
    ...
    getAgencies: function getAgencies() {
      ...
      this.$store.dispatch('agency/getData', '').then(function (response) {
        _this2.$vs.loading.close(_this2.$refs.create.$refs.content);

        _this2.agencies = response.data.data.data;
        _this2.form.agency_id = _this2.agencies[0].id;
      }).catch(function (error) {
        console.log(error);
    ...
    create: function create() {
      ...
      this.$store.dispatch('employee/create', form_data).then(function (response) {
        ...
        _this3.$router.push("/dashboard/employee/".concat(response.data.data.data.id));
